...
# Prepare the one immutable 10m-row benchmark input. This is deliberately a
# source-normalisation step, not a format-specific filter.
...
source_path <- Sys.getenv("COMPRESSOR_RAW_SOURCE", unset = "")
output_path <- Sys.getenv("COMPRESSOR_10M_SOURCE", unset = "")
metadata_path <- Sys.getenv("COMPRESSOR_10M_METADATA", unset = "")
target_rows <- as.integer(Sys.getenv("COMPRESSOR_10M_ROWS", unset = "10000000"))
...
if (nrow(raw) < target_rows) {
  stop("only ", nrow(raw), " valid biallelic SNPs; need ", target_rows)
}
...
metadata <- list(
  ...
  ordering = "lexicographic chromosome then position/ref/alt; first 10,000,000 after filtering"
)
