make_fixture <- function(n = 1200L) {
  data.frame(
    ...
    reference_allele = rep(c("C", "G", "T", "A"), length.out = n),
    alternate_allele = rep(c("A", "C", "G", "T"), length.out = n),
    ...
    variant_id = paste0("1:", seq.int(100001L, length.out = n), ":",
                        rep(c("C", "G", "T", "A"), length.out = n), ":",
                        rep(c("A", "C", "G", "T"), length.out = n)),
